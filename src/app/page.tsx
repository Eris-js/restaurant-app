...
export default async function Home() {
  ...
  return (
    <div className="min-h-screen scroll-smooth">
            ...
            {promotions.map((promo: any) => (
              <div key={promo._id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300 flex flex-col h-full border border-gray-100">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <p className="text-gray-600 mb-4 flex-grow line-clamp-3">{promo.description}</p>
      ...
      {/* Articles Section */}
      {articles.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-gray-50 py-12 rounded-2xl">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-gray-900">Latest News & Blog</h2>
            <p className="mt-2 text-gray-600">Updates from our kitchen and team</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {articles.map((article: any) => (
              <div key={article._id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition">
                <div className="h-56 bg-gray-200 group overflow-hidden">
                  <img src={article.thumbnail} alt={article.title} className="w-full h-full object-cover transform group-hover:scale-110 transition duration-500" />
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-2 line-clamp-2 text-gray-800 group-hover:text-orange-600 transition">{article.title}</h3>
                  <div className="mt-4">
                    <Link href={`/articles/${article.slug}`} className="text-orange-600 hover:text-orange-700 font-medium text-sm uppercase tracking-wide">Read More</Link>
                  </div>
                </div>
              </div>
            ))}
